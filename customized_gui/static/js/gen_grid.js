...
// MHC variables
var mhc_settings_processed = false,
    mhc_settings = null,
    mhc_show_explanations = null,
    mhc_dss_biased_suggestion = null,
    mhc_display_ending_message = false;
...
function draw(state, world_settings, new_messages, accessible_chatrooms, new_tick) {
    ...
    obj_ids.forEach(function(objID) {
        ...
        if (obj.hasOwnProperty('class_inheritance') && obj['class_inheritance'].includes("PatientAgent")) {
            patient_keys.push(objID);
        }

        // MHC edit: parse MHC frontend settings
        if (obj['name'] == 'Settings' && lv_agent_type == 'human-agent') {
            // when done show the end message
            if (obj.hasOwnProperty('trial_completed') && obj['trial_completed'] && obj.hasOwnProperty('end_message') && !mhc_display_ending_message) {
                console.log("Trial completed, adding completion text");
                mhc_display_ending_message = true;

                // add the html to the page
                var completion_html = `
                    <div id="trial_completed_text">
                        ${obj['end_message']}
                    </div>`;
                $('body').append(completion_html);
            }

            if (!mhc_settings_processed) {
                parse_mhc_settings(obj);
                mhc_settings = obj;
                mhc_settings_processed = true;
            }
        }
        ...
        // MHC-TRIAGE
        // fetch health if defined
        var obj_health = null;
        if (Object.keys(obj).includes('health')) {
...
function gen_image(obj_vis_settings, obj_element) {
    ...
    // MHC-TRIAGE
    if (obj_vis_settings['health'] != null) {
        var health = clamp(obj_vis_settings['health'], 0, 100);
        //circle health visualization
        var healthcircle_border = document.createElement("div");
        healthcircle_border.className = "mhc-healthcircle-border";


        var healthcircle = document.createElement("div");
        healthcircle.className = "mhc-healthcircle";
        if(health<25) {
            ...
            if (health == 0) {
                ...
                obj_element.append(death_ring);
            }
        } else {
            if(health < 50) {
                healthcircle.style.backgroundColor = "orange"
            }else if(health < 75){
                healthcircle.style.backgroundColor = "yellow"
            } else {
                ...
                if (health == 100 ) {
                    ...
                    obj_element.append(recovery_ring);
                }
            }
        }

        healthcircle_border.append(healthcircle);

         if(obj_vis_settings['health'] <= 0 ) {
        ...
        /*// create a healthbar wrapper
        var healthbar_border = document.createElement("div");
        healthbar_border.className = "mhc-healthbar-border";

        // create the healthbar content and fill it the correct amount
        var healthbar = document.createElement("div");
        healthbar.className = "mhc-healthbar";
        healthbar.style.width = health + "%";
        healthbar_border.append(healthbar);

        // if a person is deceased, fade it out of existence
        if(obj_vis_settings['health'] <= 0 ) {
